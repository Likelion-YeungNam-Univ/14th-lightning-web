...
const fieldClass =
  "h-12 w-full rounded-[10px] border border-[#3a4250] bg-[#12151b] px-4 text-sm text-[#f2f3f5] outline-none placeholder:text-[#737b88] focus:border-[#6f9fff]";
...
export function LoginModal({ onClose, onLoginSuccess }: LoginModalProps) {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setError("");
    if (mode === "login" && (!loginId.trim() || !password))
  ...
  return (
    ...
    >
      <section
        ...
        className="relative flex max-h-[calc(100vh-40px)] w-full max-w-[520px] flex-col overflow-hidden rounded-[18px] border border-[#303744] bg-[#1c2029] shadow-[0_28px_90px_rgba(0,0,0,.6)]"
      >
        <div className="flex shrink-0 items-center justify-between bg-[#1c2029] px-7 pb-3 pt-6 max-[640px]:px-5">
          ...
          <button
            ...
            aria-label="닫기"
            onClick={onClose}
            className="grid size-11 place-items-center rounded-full border-0 bg-[#2a2f3a] text-[28px] font-light text-[#b9c1ce] transition hover:bg-[#343b48] hover:text-white"
          >
            ×
          </button>
        </div>
        <div className="min-h-0 overflow-y-auto px-7 pb-7 max-[640px]:px-5">
          <h2
            id="auth-title"
            className="mb-0 mt-4 text-[28px] font-bold tracking-[-0.04em]"
          >
            {mode === "signup" ? "회원가입" : "로그인"}
          </h2>
          <p className="mb-6 mt-2 text-[13px] leading-6 text-[#9aa3b2]">
            {mode === "signup"
              ? "아이디와 비밀번호, 커뮤니티에서 사용할 닉네임을 만들어 주세요."
              : "아이디와 비밀번호를 입력해 주세요."}
          </p>
          <form onSubmit={submit}>
            <label
              htmlFor="auth-id"
              className="mb-2 block text-sm font-bold text-[#c8ccd4]"
            >
              아이디
            </label>
            <input
              ...
              pattern={mode === "signup" ? "[a-z0-9_]{4,20}" : undefined}
              value={loginId}
              onChange={(e) => setLoginId(e.target.value)}
              placeholder={mode === "signup" ? "예) assit_user" : "아이디 입력"}
              className={fieldClass}
            />
            {mode === "signup" && (
              <p className="mb-5 mt-2 text-xs text-[#9aa3b2]">
                영문 소문자·숫자·밑줄 중 사용, 4~20자
              </p>
            )}
            <label
              htmlFor="auth-password"
              className="mb-2 mt-5 block text-sm font-bold text-[#c8ccd4]"
            >
              비밀번호
            </label>
            <input
              ...
              <>
                ...
                <div className="flex gap-3">
                  ...
                  <button
                    ...
                    className="w-[96px] shrink-0 rounded-[10px] border border-[#5f8bd1] bg-[#21304a] text-xs font-bold text-[#a9c8ff] transition hover:bg-[#293c5c]"
                  >
                    형식 확인
                  </button>
                </div>
                ...
                {nicknameStatus && (
                  <p
                    className={`mt-2 text-xs font-bold ${nicknameStatus.startsWith("사용할 수") ? "text-[#82d5a0]" : "text-[#ef7b7b]"}`}
                  >
                    {nicknameStatus}
                  </p>
                )}
            ...
            {error && (
              <p role="alert" className="mt-5 text-sm text-[#ef7b7b]">
                {error}
              </p>
            )}
            <button
              type="submit"
              disabled={loading}
              className="mt-6 h-12 w-full rounded-[10px] border-0 bg-[#6f9fff] text-sm font-bold text-[#0f1115] transition hover:bg-[#83adff] disabled:opacity-60"
            >
              {loading
                ? mode === "signup"
                  ? "가입 중..."
          ...
          <p className="mb-0 mt-6 text-center text-sm text-[#9aa3b2]">
            {mode === "signup" ? "이미 계정이 있나요? " : "처음이신가요? "}
            <button
              type="button"
              onClick={() => changeMode(mode === "signup" ? "login" : "signup")}
              className="border-0 bg-transparent p-0 font-bold text-[#6f9fff] underline underline-offset-4"
            >
